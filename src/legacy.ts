import { HOOK_SCHEDULER } from './constant';
...
class XhrInterceptor {
  ...
  private xhrInstanceAttr = ['response', 'responseText', 'responseXML', 'status', 'statusText'];
  private xhrMethodsHandler = {
    open: function (self: XhrInterceptor, target: XMLHttpRequest) {
      return function (...args: Parameters<XMLHttpRequest['open']>) {
        const hooker: HookScheduler = target[HOOK_SCHEDULER];
        hooker.reset();
        hooker.request = {
          type: 'XHR',
          method: args[0] || 'GET',
  ...
  _generateProxyXMLHttpRequest({
      ...
      .map((attr) => {
        return {
          ...
          handler: function (self, target) {
            const hooker = target[HOOK_SCHEDULER];
    ...
    function proxyXhr() {
      ...
      xhr[HOOK_SCHEDULER] = new HookScheduler({
        hooks: hooks.slice(),
      });

      xhr.addEventListener('readystatechange', () => {
        if (xhr.readyState === 4) {
          const hooker = xhr[HOOK_SCHEDULER];
...
class FetchInterceptor {
  ...
  static getInstance() {
    if (!FetchInterceptor.#instance) {
      FetchInterceptor.#instance = new FetchInterceptor(FetchInterceptor.#token);
    }
  ...
  _generateProxyFetch({
    ...
    async function proxyFetch(url: string, options: RequestInit) {
      ...
      const hooker = new HookScheduler({
        hooks: hooks.slice(),
      });
...
class AjaxInterceptor {
  ...
  _generateProxyFetch() {
    const self = this;
    async function proxyFetch(url, options: RequestInit) {
      const winFetch = self.nativeFetch;
      const hooker = new HookScheduler({
        hooks: self.hooks.slice(),
      });
    ...
    Object.keys(self.nativeFetch).forEach((key) => {
      proxyFetch[key] = self.nativeFetch[key];
    });
    proxyFetch.prototype = this.nativeFetchPrototype;
    return proxyFetch;
  }
